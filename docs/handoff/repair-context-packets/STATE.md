...
| P13-Q | 修复第三方复核发现的验收自证漏洞：工具示例写死 `answer-source` 使真实模型照抄后全部引文绕过核验。sourceFileId 三分类（主试卷核验 / 作业内真实其它文件 unverifiable / 其它值拒绝 SOURCE_UNKNOWN）；示例与 prompt 注入真实主试卷 ID | done | `6f66e09`。(a)(b)(c)(d) 四条用例先红后绿（(a)/(d) 见红于 P12-Q 旧行为、(c) 见红于写死占位）；全新复核子代理 2 处变异变红并还原、放行；命令处理器层 + 真实 HTTP |
...
- **`MIN_TEXT_LAYER_CHARS = 8` 的边界**（P13-Q 记录，等真实样本再调）：只有一行文字标题（≤8 规范化字符）+ 扫描表格的答案页，其文本层会低于阈值——从扫描表格**图片**里读来的真实引文会因「相邻页都有有效文本层」被误拒（`CLOUD_EDIT_EVIDENCE_QUOTE_NOT_IN_SOURCE`）。阈值调低会让「薄文本层页」大量逃成 unverifiable、削弱反证能力，因此不动；等出现真实样本（该形态的答案页 + 被误拒的引文）再校准阈值或引入逐页图片覆盖率判据。
...
- 2026-09-25 P13-Q（`6f66e09`）：修复验收自证漏洞——修复输入的工具示例把 evidence 写死为 `answer-source`（既不是主试卷、也不是作业里任何真实文件），真实模型照抄示例时 P12-Q 语义下全部引文跳过比对、以 unverifiable 落库，P9 核验失效；受控假模型用请求里的真实 ID，既有测试看不到。
  - **sourceFileId 三分类**（`EvidenceSourceContext` 携带主试卷 id 与作业源文件清单）：等于主试卷 id → 照常对照完整原文核验；是 `job.sourceFiles` 里真实存在的其它文件 → 标 unverifiable（P12-Q 行为保留）；其它任何值 → 整批拒绝 `CLOUD_EDIT_EVIDENCE_SOURCE_UNKNOWN:<index>`，错误信息给出合法 id。清单读不到（无 job / 元数据缺失）时不分类、全部标 unverifiable——对「id 存不存在」下编造结论需要作业元数据做依据；生产路径每回合开头必 `load_job`（复核确认该退让不可被系统性滥用）。
  - **示例注入真实 id**：`repair_tools_table(main_source_file_id)` 四处示例改注入 `source.file_id`（`repair_authoring_step_through_gateway` 的主试卷），prompt 里不再出现任何写死占位 id。
  - **先红后绿**：(a) 编造 id「answer-source」→ P12-Q 旧行为下 Applied（临时退回旧行为见红 `left: Applied / right: Rejected`）→ 分类后 SOURCE_UNKNOWN 拒绝；(b) 作业内真实答案文件 → unverifiable 放行（P12-Q 用例改名钉住）；(c) 契约测试（两种模式、真实 HTTP，断言 prompt 里每个 sourceFileId == 主试卷 id 且无 `answer-source`）→ 写死占位时红、注入后绿；(d) 受控服务 `copyExampleSourceId` 剧本（故意从 prompt 工具示例原样抄 id——真实模型最可能的行为）：示例未修时复制 id 被拒、答案留 B（红）；修复后引文完整核验、编辑落地、`evidenceUnverifiable` 为空（绿）。
  - **复核**：全新只读对抗式子代理放行；2 处变异（三分类 contains 恒真 → (a) 红；注入参数改回占位 → (c) 红）均独立变红并精确还原。两条非阻断观察（SOURCE_UNKNOWN 在 record_ruling 路径的冒号拼接外观、主 id 空兜底不可达）记 REPORT §11。
  - 全量：`cargo test --lib` **1134/0/11**（上轮 1131，+3）、`cargo test --lib cloud_repair` **139**（136）、vitest **392**（同一 suite 因缺 `selenium-webdriver` 加载失败）、tsc `exit 0`。CDP 与真实模型仍未执行（平台 / 额度）。
