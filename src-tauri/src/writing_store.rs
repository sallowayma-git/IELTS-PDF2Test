...
use crate::util::{
    read_json, safe_writing_job_dir, validate_path_segment, write_json, writing_job_dir,
};
...
pub(crate) fn save_writing_job(root: &Path, job: &WritingJob) -> CommandResult<()> {
    ...
    write_json(
        &writing_job_dir(root, &job.job_id).join("writing-job.json"),
        job,
    )?;
    // 双写题库 DB（失败记日志但不阻断主流程）。
    if let Err(error) = crate::library_commands::upsert_writing_job(root, job) {
        eprintln!(
            "[library] upsert_writing_job failed for {}: {}",
            job.job_id, error
        );
    }
    Ok(())
...
pub(crate) fn delete_writing_job(root: &Path, job_id: &str) -> CommandResult<()> {
    ...
    if let Err(error) = crate::db::delete_exam_by_id(root, job_id) {
        eprintln!(
            "[library] delete_exam_by_id failed for writing {}: {}",
            job_id, error
        );
    }
    Ok(())
