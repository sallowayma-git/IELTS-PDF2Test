...
mod tests {
    ...
    /// （审计发现 A-13），并且必须**完整包住 bbox**。
    ///
    /// 为什么要把「需求」和「具体像素值」分开写（P7 审计 #2）：原来这一条只断言了
    /// `(88, 88)` 这个数字 —— 那是**实现值**，不是需求。数字对了也看不出它为什么该是
    /// 这个数；将来有人把边距从 10% 调成 30%，测试照样绿，而契约已经变了。
    ///
    /// 语义（刻意写明，因为这是一个选择而不是推导）：裁剪以锚点的**上边缘**为中心，
    /// 上下各给 `height + 10% × height`，即页单位高度 2.2 × bbox 高。这样无论如何都
    /// 完整包含 bbox，并在下方留 10% 的余量。旧代码把已是像素的 `top` 又加了一遍
    /// 页单位的 `height * 3.0`，裁出来的高度是 3.1 ×，比承诺的多出近一倍——模型拿到
    /// 一张「比它要的区域大得多」的图，却以为那就是证据区域。
    #[test]
    fn a_region_crop_keeps_the_bbox_height_it_promised() {
        ...
        let info = png::Decoder::new(std::io::Cursor::new(&bytes))
            ...
            .info()
            .clone();
        // 页图是 2 像素 / 点，所以像素尺寸除以 2 就是页单位尺寸。
        let width_points = info.width as f64 / 2.0;
        let height_points = info.height as f64 / 2.0;
        // 需求一：完整包住 bbox（40×20 点）。裁剪小一圈，模型看到的区域就缺一块。
        assert!(
            width_points >= 40.0,
            "裁剪宽度 {width_points} 点必须不小于 bbox 宽 40 点"
        );
        assert!(
            height_points >= 20.0,
            "裁剪高度 {height_points} 点必须不小于 bbox 高 20 点"
        );
        // 需求二：不许无谓放大。左右只多 5%、上下不超过 bbox 高的 2.5 倍
        // （2.2 × 是当前语义）。A-13 的旧实现是 3.1 ×，这条会把它判红。
        assert!(
            width_points <= 40.0 * 1.2,
            "裁剪宽度 {width_points} 点不该比 bbox 宽多出两成以上"
        );
        assert!(
            height_points <= 20.0 * 2.5,
            "裁剪高度 {height_points} 点不该比 bbox 高多出一倍以上"
        );
        // 需求都满足后，再把当前的具体值钉住：将来改动会在这里被看见，必须重新说明。
        //   x 边距 5% × 40 = 2 点、y 边距 10% × 20 = 2 点：
        //   左右 (10-2)~(10+40+2) = 8~52 点 → 16~104 像素，宽 88；
        //   上下 (50-20-2)~(50+20+2) = 28~72 点 → 56~144 像素，高 88。
        assert_eq!(
            (info.width, info.height),
            (88, 88),
            "裁剪尺寸变了：先确认需求是否也跟着变，再改这个数字。实际 {}x{}",
            info.width,
            info.height
        );
