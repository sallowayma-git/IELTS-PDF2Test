...
pub(crate) fn migrate_db_file(root: &Path) -> CommandResult<()> {
    ...
    if legacy.exists() {
        std::fs::rename(&legacy, &current)
            .map_err(|e| format!("migrate_db_rename:{}:{}", legacy.display(), e))?;
        eprintln!(
            "[library] migrated legacy DB {} -> {}",
            LEGACY_DB_NAME, DB_NAME
        );
    }
    Ok(())
}
...
pub(crate) fn list_exams(
    ...
    let mut stmt = conn
        .prepare(&sql)
        .map_err(|e| format!("list_exams_prepare:{}", e))?;
    let rows = stmt
...
pub(crate) fn get_exam(conn: &Connection, id: &str) -> CommandResult<Option<LibraryExamDetail>> {
    ...
    let mut stmt = conn
        .prepare(&sql)
        .map_err(|e| format!("get_exam_prepare:{}", e))?;
    let mut rows = stmt
        .query_map(params![id], |row| {
...
pub(crate) fn search_exams(
    ...
    let mut stmt = conn
        .prepare(&sql)
        .map_err(|e| format!("search_exams_prepare:{}", e))?;
    let rows = stmt
...
pub(crate) fn get_stats(conn: &Connection) -> CommandResult<LibraryStats> {
    ...
    fn group_count(conn: &Connection, column: &str) -> CommandResult<BTreeMap<String, u32>> {
        ...
        let mut stmt = conn
            .prepare(&sql)
            .map_err(|e| format!("stats_group_prepare:{}", e))?;
        let rows = stmt
            .query_map([], |row| {
...
fn get_exam_summary(conn: &Connection, id: &str) -> CommandResult<Option<LibraryExamSummary>> {
    ...
    let mut stmt = conn
        .prepare(&sql)
        .map_err(|e| format!("get_summary_prepare:{}", e))?;
    let mut rows = stmt
...
pub(crate) fn prune_orphan_exams(
    conn: &Connection,
    live_ids: &std::collections::HashSet<String>,
) -> CommandResult<usize> {
    let all: Vec<String> = {
        let mut stmt = conn
            .prepare("SELECT id FROM exams")
            .map_err(|e| format!("prune_prepare:{}", e))?;
        let rows = stmt
            .query_map([], |row| row.get::<_, String>(0))
            .map_err(|e| format!("prune_query:{}", e))?;
        let mut v = Vec::new();
        for r in rows {
...
pub(crate) fn upsert_library_item(
    conn: &Connection,
...
fn exam_record_from_library_item(conn: &Connection, id: &str) -> CommandResult<Option<ExamRecord>> {
    ...
    match rows.next() {
        None => Ok(None),
        Some(row) => Ok(Some(
            row.map_err(|e| format!("library_item_exam_row:{}", e))?,
        )),
    }
}
...
mod tests {
    ...
    fn sample_library_item(id: &str, subject: &str, status: &str) -> LibraryItemRecord {
        LibraryItemRecord {
            id: id.to_string(),
            subject: subject.to_string(),
            content_type: if subject == "writing" {
                "writing_task"
            } else {
                "reading_exam"
            }
    ...
    fn library_item_upsert_creates_revisions() {
        ...
        let current: String = conn
            .query_row(
                "SELECT current_revision_id FROM library_items WHERE id='lib-1'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        ...
        let count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM library_item_revisions WHERE library_item_id='lib-1'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        ...
        let max_no: i64 = conn
            .query_row(
                "SELECT MAX(revision_no) FROM library_item_revisions WHERE library_item_id='lib-1'",
                [],
                |r| r.get(0),
            )
            .unwrap();
    ...
    fn active_queries_follow_soft_delete_and_normalize_statuses() {
        ...
        assert!(list_exams(&conn, &LibraryFilter::default())
            .unwrap()
            .is_empty());
        ...
        assert_eq!(
            list_exams(&conn, &LibraryFilter::default()).unwrap().len(),
            1
        );
    }
    ...
    fn migrate_db_file_renames_legacy() {
        use std::fs;
        let dir =
            std::env::temp_dir().join(format!("ielts-migtest-{}", uuid::Uuid::new_v4().simple()));
        ...
        assert!(
            ...
            "legacy db must be renamed away"
        );
        ...
        assert_eq!(
            ...
            "existing new db must not be overwritten"
        );
