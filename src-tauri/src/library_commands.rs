...
pub(crate) fn exam_record_from_reading_job(
    job: &ImportJob,
    payload: Value,
) -> ExamRecord {
    let source_hash = job
...
pub(crate) fn update_library_exam_meta_core(
    ...
    if let Err(e) = write_back_meta_to_source(root, &before.subject, id, &patch) {
        eprintln!("[library] write_back_meta_to_source failed for {}: {}", id, e);
    }
    Ok(Some(updated))
}
...
pub(crate) fn migrate_existing_into_library(root: &Path) -> CommandResult<usize> {
    ...
    if let Ok(entries) = std::fs::read_dir(&jobs_root) {
        for entry in entries.flatten() {
            ...
            let job: ImportJob = match crate::util::read_json(&job_json) {
                Ok(j) => j,
                Err(e) => {
                    skipped.push(format!("reading job.json read failed: {}: {}", entry.path().display(), e));
                    continue;
                }
            };
            let payload = match reading_payload(root, &job) {
                ...
                Err(ReadingPayloadError::ReadFailed(e)) => {
                    ...
                    skipped.push(format!("reading authoring-ir read failed, fallback to job: {}: {}", job.job_id, e));
                    serde_json::to_value(&job).unwrap_or(Value::Null)
                }
            };
            live_ids.insert(job.job_id.clone());
            records.push(MigRecord::Reading(exam_record_from_reading_job(&job, payload)));
        }
    }
    ...
    if let Ok(entries) = std::fs::read_dir(&wjobs_root) {
        for entry in entries.flatten() {
            ...
            let job: WritingJob = match crate::util::read_json(&job_json) {
                Ok(j) => j,
                Err(e) => {
                    skipped.push(format!("writing job.json read failed: {}: {}", entry.path().display(), e));
                    continue;
                }
            };
            live_ids.insert(job.job_id.clone());
    ...
    let tx = conn.transaction().map_err(|e| format!("migrate_begin:{}", e))?;
...
mod tests {
    ...
    fn make_reading_appdata() -> PathBuf {
        let root = std::env::temp_dir().join(format!("ielts-lib-test-{}", uuid::Uuid::new_v4().simple()));
        ...
        crate::util::write_json(&crate::util::job_dir(&root, "import-test-1").join("job.json"), &job).unwrap();
        ...
        crate::util::write_json(&crate::util::job_dir(&root, "import-test-1").join("authoring-ir.json"), &ir).unwrap();
        root
    }

    fn make_writing_appdata() -> PathBuf {
        let root = std::env::temp_dir().join(format!("ielts-lib-wtest-{}", uuid::Uuid::new_v4().simple()));
        ...
        crate::util::write_json(&crate::util::writing_job_dir(&root, "writing-test-1").join("writing-job.json"), &job).unwrap();
        root
    }
    ...
    fn update_meta_writes_back_to_job_json_and_survives_resave() {
        ...
        let updated = update_library_exam_meta_core(
            &root,
            "import-test-1",
            LibraryMetaPatch { title: Some("New Title".into()), ..Default::default() },
        )
        .unwrap()
        .unwrap();
        ...
        assert_eq!(job.title, "New Title", "job.json title must be synced from library edit");
        ...
        assert_eq!(detail.summary.title, "New Title", "DB title must survive a resave (no silent overwrite)");
        cleanup(&root);
    }
    ...
    fn migrate_prunes_orphan_db_rows() {
        ...
        {
            let conn = crate::db::open_connection(&root).unwrap();
            conn.execute("DELETE FROM library_meta WHERE key='migration_done_v1'", []).unwrap();
        }
        migrate_existing_into_library(&root).unwrap();
        let conn = crate::db::open_connection(&root).unwrap();
        assert!(crate::db::get_exam(&conn, "import-orphan-ghost").unwrap().is_none(), "orphan must be pruned");
        assert!(crate::db::get_exam(&conn, "import-test-1").unwrap().is_some(), "live row must remain");
        cleanup(&root);
    }
}
