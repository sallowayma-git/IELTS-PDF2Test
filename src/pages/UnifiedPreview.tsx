...
function buildSrcDoc(assets: PreviewAssets): string {
  return `<!doctype html><html><head><meta charset="utf-8"><style>body{font-family:Georgia,serif;margin:0;padding:24px;background:#f5f1e8;color:#17211f;line-height:1.6}.layout{display:grid;grid-template-columns:1fr 420px;gap:28px}.pane{background:#fffaf0;border:1px solid #d8cfbf;padding:22px}.choice-row{display:flex;gap:10px;flex-wrap:wrap}.completion-table{width:100%;border-collapse:collapse}.completion-table th,.completion-table td{border:1px solid #c8beaa;padding:8px}input{font:inherit;padding:6px}</style></head><body><div class="layout"><article class="pane">${assets.source.passage.blocks.map((block) => block.html).join("")}</article><section class="pane">${assets.source.questionGroups.map((group) => group.bodyHtml).join("")}</section></div></body></html>`;
}
  ...
  return (
    <section className="page-enter">
      <div className="section-heading spread">
        ...
        <div className="button-row"><button className="ghost" onClick={generate}>重新生成预览</button><button className="primary" disabled={!assets} onClick={e2e}>自动填正确答案 E2E</button><button className="ghost" onClick={() => go(`/jobs/${jobId}/export`)}>导出</button></div>
      </div>
      <div className="preview-grid">
        <iframe title="reading-preview" srcDoc={assets ? buildSrcDoc(assets) : "<p>Generate preview assets first.</p>"} />
        <aside className="inspector">
          <p className="eyebrow">Validation</p>
          <h3>{report?.passed ? "通过" : "待校验"}</h3>
          <div className="layer-list">{report?.layers.map((layer) => <div key={layer.layer}><span>{layer.layer}</span><strong>{layer.passed ? "pass" : `${layer.issueCount} issues`}</strong></div>)}</div>
