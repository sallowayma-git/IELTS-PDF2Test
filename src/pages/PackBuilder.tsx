...
export function PackBuilder({ jobs, refresh }: { jobs: ImportJob[]; refresh: () => void }) {
  const publishable = useMemo(() => jobs.filter((job) => ["PreviewReady", "ExportReady", "Published"].includes(job.status)), [jobs]);
  ...
  return (
    <section className="page-enter">
      <div className="section-heading spread"><div><p className="eyebrow">Pack Builder</p><h2>组卷与发布</h2></div><button className="primary" disabled={!selected.length} onClick={run}>生成 Pack</button></div>
      <div className="pack-grid">
        <section className="form-section"><h3>可发布题库</h3>{publishable.map((job) => <label className="pick-row" key={job.jobId}><input type="checkbox" checked={selected.includes(job.jobId)} onChange={(event) => setSelected((current) => event.target.checked ? [...current, job.jobId] : current.filter((id) => id !== job.jobId))} /><span>{job.title}</span><StatusPill status={job.status} /></label>)}{!publishable.length ? <p className="empty">没有 PreviewReady/ExportReady 的题目。</p> : null}</section>
        ...
        <aside className="inspector"><p className="eyebrow">Result</p><h3>{result?.packId ?? "未生成"}</h3>{error ? <p className="error-text">{error}</p> : null}<pre>{JSON.stringify(result, null, 2)}</pre></aside>
      </div>
    </section>
  );
}
