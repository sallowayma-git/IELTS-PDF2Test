  ...
  return (
    <section className="document-review page-enter">
      <div className="section-heading spread">
        ...
        <div className="button-row"><button className="ghost" onClick={ocr}>重新 OCR</button><button className="ghost" disabled={visionBusy} onClick={visionTranscribe}>{visionBusy ? "视觉转录中..." : "视觉 LLM 转录"}</button><button className="ghost" disabled={!sourceReview?.required || sourceReview.resolved} onClick={resolveReview}>确认源文档审核</button><button className="primary" onClick={split}>进入粗切</button></div>
      </div>
      <div className="review-grid">
        ...
        <aside className="inspector">
          ...
          <dl>
            ...
            <dt>source review</dt><dd>{sourceReview?.required ? (sourceReview.resolved ? "resolved" : "required") : "not required"}</dd>
          </dl>
          {sourceReview?.required ? <><h4>Source Review Gate</h4><pre>{JSON.stringify(sourceReview, null, 2)}</pre></> : null}
          <details open={Boolean(sourceReview?.required || lowConfidenceBlocks.length || parserWarnings.length)}>
            ...
            <button className="ghost wide" disabled={visionBusy} onClick={visionTranscribe}>{visionBusy ? "视觉转录中..." : "用视觉大模型生成转录稿"}</button>
            ...
            <label>转录文本<textarea value={manualText} onChange={(event) => setManualText(event.target.value)} placeholder="粘贴 passage、questions、answers；保留 Questions 1-5 / Answers 这类标题有助于自动切分。" /></label>
            <button className="primary wide" disabled={!manualText.trim()} onClick={applyTranscription}>应用手工转录</button>
          </details>
          {parserWarnings.length ? <><h4>Parser Warnings</h4><pre>{JSON.stringify(parserWarnings, null, 2)}</pre></> : null}
          {lowConfidenceBlocks.length ? <><h4>低置信块</h4><pre>{JSON.stringify(lowConfidenceBlocks.map((block) => ({ blockId: block.blockId, confidence: block.confidence, roleHint: block.roleHint, text: block.text })), null, 2)}</pre></> : null}
          {pipelineReport ? <><h4>Auto Pipeline</h4><pre>{JSON.stringify({ status: pipelineReport.status, currentStep: pipelineReport.currentStep, parser: pipelineReport.parser, llm: pipelineReport.llm, runtimeMode: pipelineReport.runtimeMode }, null, 2)}</pre></> : null}
