 ...
 * - 听力稿：按 listeningParts(authoring, []) 的 Part 划分 section；section 的题目集合
 *   与 ExamCanvas 的 shownTaskIds（visibleTaskIds）显示语义保持一致，切 Part 就是
...
export interface QuestionNavInput {
  ...
  /** 听力稿的 Part 视图（ExamCanvas 传 listeningParts(authoring, [])）；缺省视为阅读稿。 */
...
export function buildQuestionNavModel(input: QuestionNavInput): QuestionNavModel {
  ...
  const hasPartMapping = Boolean(parts?.some((part) => part.taskIds.length > 0));
  if (!parts?.length || !hasPartMapping) {
    // 阅读稿；或听力稿没有任何 Part → 题组映射（页面上本来就显示全部题组）：
    // 都退化为单一 "Part 1"。
  ...
  // 听力稿：映射了题组的 Part 只列自己的题目；没映射的 Part 与 visibleTaskIds
  // 的行为一致（显示全部题组），保证导航计数和右侧题面永远对得上。
